...
function rollPatrolEvents(idx, scene = null) {
  ...
  const maxSteps = Phaser.Math.Between(28 + area.dungeonNo * 2, 40 + area.dungeonNo * 3);
  const picked = [];
...
function startExploreRun(scene, idx) {
  ...
  scene.board.exploreRun = { idx, boss, event: 0, line: 0, timer: 0, steps: 0, maxSteps: nodes.maxSteps || 32, nodes, shown: [`${boss.name}이 ${area.name} 순찰을 시작했다.`], rewards: { soul: 0, notoriety: 0, monsters: 0, clues: 0, exp: 0, levelUps: 0 }, hp: boss.hp, maxHp: boss.maxHp, finished: false };
